...
def create_grid(buffer):
    bounds = buffer.geometry.total_bounds
    stop_id = str(buffer['stop_id'].item())

    x_gap = bounds[2] - bounds[0]
    x_points = ceil(x_gap / ft500)
    x_coords = [bounds[0]+(x*ft500) for x in range(0, x_points)]

    y_gap = bounds[3] - bounds[1]
    y_points = ceil(y_gap / ft500)
    ...
    grid['grid_id'] = stop_id + "_" + grid['grid_id'].astype(str)
